...
def prepare_cp_csv(cp_dict, confidence=0.95):
    rows = []
    for model_name, data in cp_dict.items():
        ...
        coverage_ci  = 1.96 * np.std(coverage_per_sample, ddof=1) / np.sqrt(n)

        # interval width per sample
        width = upper - lower
        width_mean = np.mean(width)
        width_ci  = 1.96 * np.std(width, ddof=1) / np.sqrt(n)

        rows.append({
            "model": model_name,
            "coverage_mean": coverage_mean,
            "coverage_95CI": coverage_ci,
            "interval_mean_width": width_mean,
            "interval_width_95CI": width_ci
        })
...
def main():
    ...
    # 2. Carica dataset CIFAR10 ridotto
    (train, cal, test) = load_cifar10_splits(
    ...
    # 3. Carica modelli RobustBench
    models = load_models_from_config(config)

    # 4. Genera gli avversari AutoAttack
    adv_data = {}
    ...
    print("\nDone generating adversarial data.")
    
    print("\nCalculating scores.")

    clean_results = extract_scores(models, cal_X, cal_y, test_X, test_y, datamode="clean")
    adv_results   = extract_scores(models, cal_X_adv, cal_y, test_X_adv, test_y, datamode="adv")
    
    print("\nCalculating vanilla cp.")

    vanilla_cp_test = compute_cp(
        clean_results,
        adv_results,
        alpha=config["alpha"],
        cal_on='clean',
    ...
    print("\nCalculating adversarial cp.")
    
    adv_cp_test = compute_cp(
        clean_results,
        adv_results,
        alpha=config["alpha"],
        cal_on='adv',
    ...
    df_all = pd.concat([df_vanilla, df_adv], axis=0)
    ...
    print(f"CP results saved to {csv_path}")
