...
def load_cifar100_splits(
    ...
    """
    Carica piccoli split (train/cal/test) da CIFAR-100,
    mantenendo la struttura della tua funzione originale.
    """
    ...
    idx_test = perm_train[n_train+n_cal : n_train+n_cal+n_test]
...
def load_rb_model(model_name, dataset="cifar10", device="cuda" , norm = 'corruptions', model_dir="./models"):
...
def main():
    ...
    # 1. Carica config
    config = load_config("config_cifar100.yaml")
    eps        = config["eps"]
    ...
    (train, cal, test) = load_cifar100_splits(
    ...
    for model_name in config["models"]:
        ...
        model = load_rb_model(model_name, 'cifar100', 'cuda',norm = config["corruption"], model_dir=output_dir)
